...
int Can::parse_canframe(char *cs, struct canfd_frame *cf) {
	...
	if((cs[idx] == 'R') || (cs[idx] == 'r')){ /* RTR frame */
		...
		if(cs[++idx] && (tmp = asc2nibble(cs[idx])) <= CAN_MAX_DLC) cf->len = tmp;
	...
	for (i=0, dlen=0; i < maxdlen; i++){
		...
		if(cs[idx] == DATA_SEPERATOR) idx++; // skip (optional) separator

		if(idx >= len) break; // end of string => end of data
...
void Can::sendPackets(vector <CanFrame *>pkts) {
	...
	for(vector<CanFrame *>::iterator it = pkts.begin(); it != pkts.end(); ++it) {
		...
		cf.len = pkt->len;
		printf("> %03X [%d] ", cf.can_id, cf.len);
		if(cf.can_id>0x7ff) cf.can_id|=CAN_EFF_FLAG;
		for(i=0; i < pkt->len; i++) {
